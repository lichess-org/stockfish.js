...
ifeq ($(COMP),emscripten)
	comp=clang
	CXX=em++
	LDFLAGS += -s TOTAL_MEMORY=33554432 --memory-init-file 0 -s NO_EXIT_RUNTIME=1 -s EXPORTED_FUNCTIONS="['_main', '_uci_command']" -s "EXTRA_EXPORTED_RUNTIME_METHODS=['ccall']" --pre-js pre.js
	ifeq ($(ARCH),js)
		LDFLAGS += -s WASM=0 -s LEGACY_VM_SUPPORT=1
	else
		CXXFLAGS += --llvm-lto 3
		LDFLAGS += -s WASM=1 -s "BINARYEN_TRAP_MODE='allow'" -s BINARYEN_ASYNC_COMPILATION=1 --llvm-lto 3
	endif
endif
...
### 3.2.1 Debugging
CXXFLAGS += -DANTI -DATOMIC -DCRAZYHOUSE -DHORDE -DKOTH -DRACE -DTHREECHECK
...
objclean:
	@rm -f $(EXE) stockfish.js stockfish.wasm *.o ./syzygy/*.o
